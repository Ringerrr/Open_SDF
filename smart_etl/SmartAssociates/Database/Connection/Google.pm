...
my $IDX_AUTH_CONFIG                           =  SmartAssociates::Database::Connection::Base::FIRST_SUBCLASS_INDEX + 0;
my $IDX_BUCKET_CACHE                          =  SmartAssociates::Database::Connection::Base::FIRST_SUBCLASS_INDEX + 1;

use constant FIRST_SUBCLASS_INDEX             => 2;

use constant DB_TYPE                          => 'Google';

# Subclasses must override connect_do() and _get_bucket_client()

sub auth_config {
    my $self = shift;
    if ( @_ ) {
        $self->[ $IDX_AUTH_CONFIG ] = shift;
    }
    return $self->[ $IDX_AUTH_CONFIG ];
}

sub bucket_cache {
    my $self = shift;
    if ( !defined $self->[ $IDX_BUCKET_CACHE ] ) {
        $self->[ $IDX_BUCKET_CACHE ] = {};
    }
    return $self->[ $IDX_BUCKET_CACHE ];
}
...
sub GOOGLE_CLOUD_STORAGE_GET {
    ...
    if ( ! defined $target_file_part or $target_file_part eq '' ) {
        ...
        $final_filename = $s3_key_filename_part;
    } else {
        $target_dir = $target_dir_part;
        $final_filename = $target_file_part;
    }
    ...
    if ( @{$create_errors} ) {
        ...
        eval {
            my $bucket = $self->_get_bucket_client( $bucket_name );
            # Google::Cloud::Storage::Bucket API requires downloading to a directory
            # with the source filename, then we rename if needed
            $bucket->download_file( $key, $target_dir );

            # If the downloaded filename differs from our target, rename it
            my $downloaded_path = File::Spec->catfile( $target_dir, $s3_key_filename_part );
            my $target_full_path = File::Spec->catfile( $target_dir, $final_filename );

            if ( $downloaded_path ne $target_full_path && $final_filename ne $s3_key_filename_part ) {
                rename( $downloaded_path, $target_full_path )
                    or die "Failed to rename $downloaded_path to $target_full_path: $!";
            }

            $record_count = 1;
        };
        $error_string = $@;
        $record_count = 0 if $error_string;
    }
...
sub GOOGLE_CLOUD_STORAGE_PUT {
    ...
    my ( $bucket_name , $target_key , $source_file_path , $content_type , $TEMPLATE_TEXT );
    ...
    eval {
        ...
        # Parse target_key to extract directory path if present
        my ( $filename, $directory ) = fileparse( $target_key );
        $directory = undef if $directory eq './';  # Remove current directory notation

        my $object = $bucket->upload_file( $source_file_path, $content_type, $directory );

        if ( ! defined $object ) {
            die( "Call to upload_file() didn't return an object ..." );
        }
...
sub GOOGLE_CLOUD_STORAGE_DELETE {
    ...
    my ( $bucket_name , $target_key , $TEMPLATE_TEXT );
...
sub GOOGLE_CLOUD_STORAGE_ITERATOR {
    ...
    eval {
        ...
        my $bucket_name = $template_config_class->resolve_parameter( '#P_BUCKET_NAME#' )
            || die( "Missing parameter [#P_BUCKET_NAME#]" );

        my $prefix = $template_config_class->resolve_parameter( '#P_PREFIX#' );

        my $bucket = $self->_get_bucket_client( $bucket_name );

        my $response = $bucket->list_files( $prefix );
        ...
        # Google::Cloud::Storage::Bucket returns { items => [...] }
